...
// S_code for UBM-Tech-Global sites updated 2017-06-27 by Jon Ritchie


var s_account="cmpglobalvista" 
var s=s_gi(s_account) // Sends data to this Report Suite unless "s.dynamicAccountSelection=true" below

/************************** CONFIGURATION SETTINGS **************************/
s.charSet="UTF-8" // Character Encoding (Default is ASCII characters)
s.currencyCode="USD" // Currency setting for Revenue
s.trackInlineStats=true // Enables ClickMap report (data is sent about the location of all clicks)
s.trackDownloadLinks=true // Enables File Download report (data is sent each time a visitor clicks a file download link)
s.linkDownloadFileTypes="exe,zip,wav,mp3,mp4,mov,mpg,mpeg,gif,tif,png,jpg,avi,wmv,pdf,txt,doc,docx,csv,xls,xlsx,ppt,pptx" // Download file types
s.trackExternalLinks=true // Enables Exit Link report (data is sent each time a visitor clicks on an exit link)
s.linkLeaveQueryString=false // Excludes the query string parameters from the Exit Link report 
s.linkInternalFilters="javascript:"+ // Lists the domains within Tech-Global. Clicks to other doamins will be considered and exit (and seporate visit)
",ubm.com,createyournextcustomer.com,ubmcanon.com,ubmadtech.com,ubm.net,tech.ubm.com" + // 1-Corporate Sites
",enterpriseconnect.com,gtec.ca,towersummit.com,onlinemarketingsummit.com,voicecon.com" + //2-Tech-EnterpriseCommunications
",informationweek.com,darkreading.com,networkcomputing.com,drdobbs.com,allanalytics.com,nojitter.com,interop.com,banktech.com,insurancetech.com,wallstreetandtech.com,thesmbauthority.com,informationweek.stfi.re,darkreading.stfi.re,networkcomputing.stfi.re" + // 3-Tech-EnterpriseIT
",gamasutra.com,gdconf.com,indiegames.com,gdcvault.com,gamecareerguide.com,igf.com,vrdconf.com,gamesetwatch.com,gdceurope.com,igfmobile.com,gamasutra.stfi.re,gamecareerguide.net,gdceurope.co.uk,gamechoiceawards.com,cmpgame.com,gdceurope.dev,gdceurope.oc.uk,gdc.meetingsprogramme.com" + // 4-Tech-Games
",blackhat.com,insecurity.com" + // 5-Tech-InformationSecurity
",thinkhdi.com,hdiconference.com,servicemanagementfusion.com,icmi.com,servicemanagementfusion.stfi.re,hdiconnect.com" + //6-Tech-TechServices
",designcon.com,armtechcon.com" + // 7-AdMan Site
"techweb.com,canondev.com,entconnect.ca.canondev.com" // 8-Systems and test sites

/*Variable to be included if exit or download link is clicked. List of variables that might change value without loading a new page*/
s.linkTrackVars="events,eVar20,eVar21,eVar23,eVar33,prop66"
s.linkTrackEvents="event8,event21,event23,event24,event25,event26,event30,event33,event43,event44,event45,event46,event47,event48,event49,event50,event70,event71"


/* Visitor ID > Sets the Omniture user ID */
s.varVisID=s.prop21; // Sets Prop21 to the visitor ID in omniture
s.varVisID=s.eVar56; // Sets eVar56 to the visitor ID in omniture

/* Variables set with Plug-in functionality */
s.usePlugins=true
function s_doPlugins(s)
{
	/* Server > Sets the Domain of the page */
	s.eVar13=document.domain; // Sets v13 to the domain of the Page

	/* URL > Sets the URL of the page */
	s.prop23=document.URL; // Sets Prop23 to the URL of the Page
	s.eVar61=s.prop23; // Copies the result to v61

	/* Tab (Document Name) > Set eVar2=document.title */
	s.eVar2=document.title;

	/* Hierarchy > Uses plugin to set Hierachy1 from folders in the URL */
	s.hier1=getHier(); // Sets Hierarcy
	s.prop30=s.hier1; // Copies Hierarchy to p30

	/* Tracking Code > Uses plugin to set the Campaign to the Tracking Code in the URL */
	s.eVar47=s.getQueryParam('_mc'); // check URL for TCG Marketing Channel query strings
	s.eVar47=s.eVar47.toLowerCase(); // changes the results to lower case
	s.campaign=s.getQueryParam('utm_content,utm_campaign,utm_medium,utm_source,tag,cid,_mc,',':'); // check URL for 7 Marketing Channel query strings
	s.campaign=s.campaign.toLowerCase(); // changes the results to lower case
	s.mc=s.eVar47.substring(0,4);
	s.eVar49=s.crossVisitParticipation(s.mc,'s_ev51','180','10','>','',0); // Running list of tracking codes (last 180 days, 10 max codes)

	/* Internal Tracking code > Uses plugin to set p18 and v7 */
 	s.prop18=s.getQueryParam('itc'); // Check URL for internal tracking code query string
	s.prop18=s.prop18.toLowerCase(); // changes the results to lower case
	s.eVar7=s.prop18; // Copies the result to v7
	
	/* Internal Search Term > Uses plugin to set p01 internal search term */
	if(s.prop1 && s.prop1!="") s.prop1=s.getQueryParam('q,queryText,keyword,query,search,search_text',':'); // check URL for 6 search query strings

	/* Day | Time > Uses plugin to set p13 Time Parting */
  	s_hour=s.getTimeParting('h','-5'); // Identifies the hour of day
  	s_day=s.getTimeParting('d','-5'); // Identifies the day of week
  	s_timepart=s_day+"|"+s_hour; // Combines the day and hour
  	s.prop13=s_timepart.toLowerCase(); // Sets p13 to day | hour and changes to lower case
 	s.eVar10=s.prop13; // Copies the result to v10

 	/* New | Repeat Visitors > Uses plugin to set p11 and v17 */
	s.prop17=s.getNewRepeat(); // Identifies if the visit noumber is 1 or more
	s.eVar17=s.prop17; // Copies the result to v17

	/* % Page Viewed > Uses plugin to set p40 */
	//s.prop40=s.getPercentPageViewed(); // Identifies the percentage of page on the screen upon page load

	/* Days Since Last Visit > Uses plugin to set p22 and v15 */
	s.prop22=s.getDaysSinceLastVisit('s_lv'); // Checks date of user's last visit to the site
	s.eVar15=s.prop22; // Copies the result to v15

	/* Form ID > Uses plugin to set v26 */
	s.eVar26=s.getQueryParam('f,formID',':'); // Checks URL for form ID query string
	
	/* Eloqua IDs > Uses plugin to set v44 and v45 internal search term */
	s.eVar44=s.getQueryParam('elq_cid'); // Checks URL for Eloqua Contact ID query string 
	s.eVar44=s.eVar44.toLowerCase(); // changes the results to lower case
	s.eVar45=s.getQueryParam('elq_mid'); // Checks URL for Eloqua Asset ID query string 
	s.eVar45=s.eVar45.toLowerCase(); // changes the results to lower case

	/* Page Load Time > Uses plugin to set p42 */
	s.prop41=s_getLoadTime()/10; // Compares script start and end time to get page load duration. Originally 10ths of seconds, divided to full seocnds
	s.prop41=Math.ceil(s.prop41) // Rounded to simplify display

	/* Site Section > To be developed */
	//prop5 in media is the taxoomy token, in drupal is ??
	//eVar1 in media is ???, in drupal is aurgument #1
	s.eVar40=s.channel; // Could be removed, don;t need second evar for site section

	/* Author > Uses plugins to preset article variables */
	/* props: 2, 4, 7, 11, 20 must be set by CMS */
	s.eVar12=s.prop2; // Copies Article ID from p2 to v12
	//prop4 Headline has no eVar equivolent
	s.eVar28=s.prop7; // Copies Author from p7 to v28
	s.eVar37=s.prop11; // Copies Content Type from p11 to v37
	//prop20 Publish Date has no eVar equivolent
	if(s.prop7 && s.prop7!="") s.prop15=s.prop7 + " | " + s.prop4; // If p7 is not null, Concatenate Author and Headline
	s.eVar24=s.prop15; // Copies the result to v24
	if(s.prop7 && s.prop7!="") s.eVar25=s.eVar13 + " | " + s.prop7 + " | " + s.prop4; // If p7 is not null, Concatenate Domain, Author and Headline
	if(s.prop2 && s.prop2!="") s.prop3=s.eVar13 + " | " + s.prop2 + " | " + s.prop4; // If p2 is not null, Concatenate Domain, Article ID and Headline
	
	/* The following variables are set by Processing Rules	
	prop1  Internal Search term
	prop23 Page URL -- REDUNDANT
	prop18 Tracking Code - Internal Links -- REDUNDANT
	prop33 User Account: Logged-In
	eVar1  Site section
	eVar7  Tracking Code - Internal Links -- REDUNDANT
	eVar11 Group:domain
	eVar12 Article ID -- REDUNDANT
	eVar23 User Account: Logged-In 
	eVar25 Site | Author | Headline -- REDUNDANT
	eVar33 User Account: Logged-In 
	eVar37 Content Type  -- REDUNDANT
	eVar37 Purchase Number
	eVar41 DeusM Profile ID
	eVar44 Eloqua Contact ID -- REDUNDANT
	eVar45 Eloqua Asset ID -- REDUNDANT
	eVar56 Omniture visitor ID
	eVar61 Page URL -- REDUNDANT
	eVar69 HootSuite Social Post
	*/
}
/* End Plugin Config */
...
/* 2017-05-19 Plugin: Days since last Visit 1.1 - capture time from last visit */
...
/* 2017-03-22 Plugin: getVisualWebSiteOptimizer VWO clickmapper */
try {
...
/* 2017-05-19 Plugin: getQueryParam 2.3 - capture tracking tags passed in the URL */
...
/* 2017-05-19 Plugin: getTimeParting 3.0 - Set timeparting values based on time zone */
...
/* 2017-05-19 Get Visit Start - Not needed - part of base functionailty - used to determine first page of a visit */
s.getVisitStart=new Function("c",""
+"var s=this,v=1,t=new Date;t.setTime(t.getTime()+1800000);if(s.c_r(c"
+")){v=0}if(!s.c_w(c,1,t)){s.c_w(c,1,0)}if(!s.c_r(c)){v=0}return v;");

/* 2017-05-19 Plugin: getVisitNum - version 3.0 - Not needed - part of base functionailty */
...
/* 2017-05-19 Plugin: getNewRepeat 1.0 - Return whether user is new or repeat */
...
/* 2017-05-19 Plugin: getPreviousValue_v1.0 - return previous value - not in use - Captures the value of a Analytics variable on the next page view*/
...
/* Get User Segment: v1.0 - split users into various segments - not in use */
...
/* 2017-05-19 Plug-in: crossVisitParticipation v1.7 - stacks values from specified variable in cookie and returns value */
...
/* Utility Functions: append list, p_c, p_gh, split, replace, join */
...
/* 2017-05-19 Plugin: getPercentPageViewed v1.71 - amout of page visable at page load 
...
/* 2017-05-19 Plugin: getFullReferringDomains - Not needed - part of base functionailty */  
...
/* Plugin: getPageName v2.1 - - part of base functionailty*/
...
/* 2017-05-19 s_getLoadTime v1.36 - Get page load time in units of 1/10 seconds */
...
s.trackingServer="cmp.d3.sc.omtrdc.net" // Determined by Adobe account manager acss@adobe.com server to receive data packets
s.visitorNamespace="cmp" // Determined by Adobe account manager acss@adobe.com
...
/************* ADOBE TRACKING SCRIPT VERSION H.27.5 **************/
