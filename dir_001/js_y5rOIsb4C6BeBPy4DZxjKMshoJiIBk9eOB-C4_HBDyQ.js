...
;
/* @TODO rewrite this to work with iframe ads
(function ($) {

  Drupal.hitn_processAds = Drupal.hitn_processAds || {};

  Drupal.behaviors.adsRespons = {
    attach: function (context) {
      $('body').once('hitn_ads', function () {
        Drupal.hitn_processAds($('.group-main-content-wrapper'));
      });
    }
  };

  Drupal.hitn_processAds = function ($parent) {

    var mobile_app_browse = getCookie("mobile_app_browse");

    if (mobile_app_browse == "") {

      var move_ads = !$('body').hasClass('node-type-slideshow') && !$('body').hasClass('node-type-video') && !$('.fullwidth-body').length;
      if (move_ads) {
        //bottom ad
        ...
        if ($parent.find('.field-name-field-sponsor-term').length) {
          ...
          if ($('#block-hitn-display-right-sponsor-block').length) {
            additional_content_height += $('#block-hitn-display-right-sponsor-block').height();
          }
        }
        //Commenting out the following line to remove the position for right ad 2 for HITN-464 sticky right ads. - JP
        //if ($parent.find('.group-main-content').height() - additional_content_height > 850) {
